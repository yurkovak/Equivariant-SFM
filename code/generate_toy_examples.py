...
class ToyGenerator:
    ...
    def generate(self, scene_name: str, cameras_src_name: str, visualize: bool = True, target_fraction=None):
        if scene_name == 'line':
        ...
        np.savez(os.path.join(self.data_path, f'{scene_name} [CAM {cameras_src_name}].npz'), **scene_data)
        if visualize:
            self.visualize_data(scene_data, scene_name, cameras_src_name)

    def _get_scene_data(self, world_points: np.ndarray, cameras_src_name: str, target_fraction: tuple = None) -> dict:
        scene_data = {}
        ...
        scene_data['M'] = geo_utils.remove_empty_tracks_cams(M)
        return scene_data
